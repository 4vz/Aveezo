...
public sealed class SqlCell : IPrintable
{
    ...
    private T? GetNullableNumericType<T>() where T : struct => IsNull || !isNumeric ? null : Type == typeof(T) ? (T)value : value.TryConvert<T>(out var toValue) ? toValue : null;
    ...
    public T[] GetArray<T>(Func<object, T> func)
    {
        if (isArray && !IsNull)
            return values.Each(func);
        else
            return null;
    }
