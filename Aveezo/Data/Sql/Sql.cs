...
namespace Aveezo
{
    ...
    public sealed class Sql
    {
        ...
        private void ProcessException(string sql, SqlResultCollection result, Exception exception)
        {
            if (exception != null)
            {
        ...
        internal SqlResultCollection FormatedQuery(string sql, int limit, int offset, SqlOrder order)
        {
            ...
            if (limit > 0 || offset > 0)
            {
                ...
                if (order != null)
                {
                    ...
                    {
                        result.Exception = new SqlException(new InvalidOperationException(), sql);
                        return result;
                    }
                }
                else
            ...
            stopwatch.Start();
            Connection.Query(executedsql, result, SqlQueryType.Reader, out Exception exception, Timeout);
            stopwatch.Stop();
        ...
        public SqlResultCollection Query(string sql, params object[] args)
        {
            return FormatedQuery(Format(sql, args), 0, 0, null);
        }

        public SqlResultCollection PaginationQuery(string sql, int limit, params object[] args) => PaginationQuery(sql, limit, 0, null, args);

        public SqlResultCollection PaginationQuery(string sql, int limit, SqlOrder order, params object[] args) => PaginationQuery(sql, limit, 0, order, args);

        public SqlResultCollection PaginationQuery(string sql, int limit, int offset, params object[] args) => PaginationQuery(sql, limit, offset, null, args);

        public SqlResultCollection PaginationQuery(string sql, int limit, int offset, SqlOrder order, params object[] args)
        {
            return FormatedQuery(Format(sql, args), limit, offset, order);
        }

        internal SqlResultCollection FormatedExecute(string sql)
        {
            var result = new SqlResultCollection();
            var stopwatch = new Stopwatch();
        ...
        public SqlResultCollection Execute(string sql, params object[] args)
        {
            return FormatedExecute(Format(sql, args));
        }

        public SqlResultCollection Scalar(string sql, params object[] args)
        {
            var result = new SqlResultCollection();
            var stopwatch = new Stopwatch();
        ...
        public SqlInsertTable Insert(string table, params string[] columns) => table != null ? new SqlInsertTable(table, this, columns) : null;

        public SqlUpdate Update() => new(this);

        public SqlUpdateEntry Update(string table) => table != null ? new SqlUpdateEntry(this, table) : null;

        public SqlUpdateTable Update(string table, string whereColumn) => table != null ? new SqlUpdateTable(table, this, whereColumn) : null;

        public SqlDelete Delete() => new(this);

        public SqlDeleteTable Delete(string table, string whereColumn) => table != null ? new SqlDeleteTable(table, this, whereColumn) : null;
        ...
        public SqlDataCollection<T> Pull<T>(SqlCondition where) where T : SqlData, new()
        {
            ...
            var select = Select(info.Columns.Cast((column) => table[column])).From(table);
